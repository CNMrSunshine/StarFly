...
void SFGetTrustedInstaller() {
    NTSTATUS status;
    ULONG bufferSize = 1024 * 1024;
    PVOID buffer = malloc(bufferSize);
    status = SFNtQuerySystemInformation(SystemProcessInformation, buffer, bufferSize, &bufferSize);
    PSYSTEM_PROCESS_INFORMATION processInfo = (PSYSTEM_PROCESS_INFORMATION)buffer;
    SFPrintStatus("Searching for a Proper Process to Steal Access Token.", "正在寻找TrustedInstaller.exe进行访问令牌窃取");
    while (1) {
        ...
        if ((hProcess != 0xcccccccccccccccc) && (hProcess != 0) && (wcsstr(processInfo->ImageName.Buffer, L"TrustedInstaller.exe"))) {
            ...
            if (hToken != 0xcccccccccccccccc && hToken != 0) {
                if (processInfo->SessionId == 0) { 
                            ...
                            printf("----------------------------------------\n");
                            SFPrintStatus("Detected TrustedInstaller.exe", "发现TrustedInstaller.exe");
        ...
        SFPrintError("Failed to Obtain TrustedInstaller Token.", "获取TrustedInstaller令牌失败");
        free(buffer);
    }